...
const TwitCard = ({ twit }) => {
  ...
  const handleShare = async () => {
    const url = `${window.location.origin}/twit/${twit.id}`;
    try {
      if (navigator.share) {
        await navigator.share({ title: 'Admiree', text: 'Check out this post on Admiree', url });
      } else if (navigator.clipboard) {
        await navigator.clipboard.writeText(url);
        // noop UI feedback; could add snackbar in future
      }
    } catch (e) {
...
<video className="max-h-[40rem] p-5"  controls src={twit.video}/>
            ...
            <div className="py-5 flex flex-wrap justify-between items-center">
              ...
              <div className="flex items-center text-gray-600 cursor-pointer" onClick={handleShare} title="Share">
                <FileUploadIcon />
